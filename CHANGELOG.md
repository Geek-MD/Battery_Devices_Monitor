...
### Fixed
- **Clean-install sensor entity creation/ID consistency**: the main sensor now keeps the legacy naming mode so fresh installations create the expected entity ID (`sensor.battery_monitor_status`) instead of a device-name-prefixed variant.
...
### Added
- **French translation** (`translations/fr.json`): full UI and service translations in French.
...
### Added
- **Portuguese translation** (`translations/pt.json`): full UI and service translations in Brazilian Portuguese.
- **German translation** (`translations/de.json`): full UI and service translations in German.
...
### Fixed
- **HACS validation error**: removed invalid `domains` and `iot_class` fields from `hacs.json`. These fields are only valid in `manifest.json` and were causing `<Validation hacsjson>` to fail with "extra keys not allowed".
